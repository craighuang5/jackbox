...
# Construct the full path to hue.png
hue_image_path = os.path.join(current_directory, "hue.png")
# Load the hue image
hue_image = cv2.imread(hue_image_path)
########################################################################################################################
brushThickness = 10
eraserThickness = 30
...
imgCanvas = np.zeros((cameraHeight, cameraWidth, 3),
                     np.uint8)  # Initialize blank canvas
...
def generate_frames():
    ...
    detector = handTrackingModule.handDetector(
        maxHands=1, detectionConfidence=0.85)
    xp, yp = 0, 0
    ####################################################################################################################
    ...
    if hue_image is None:
        ...
        hue_image = cv2.resize(
            hue_image, (cameraWidth, cameraHeight // 20), interpolation=cv2.INTER_AREA)
        hue_image_opacity = 1
    ...
    while True:
        ...
        if len(landmark_list) != 0:
            ...
            thumb_x, thumb_y = landmark_list[4][1:]
            cv2.circle(img, (index_x, index_y), int(
                selectedThickness // 2), currentDrawColor, 2)
            if drawing:
                cv2.line(imgCanvas, (xp, yp), (index_x, index_y),
                         currentDrawColor, int(selectedThickness))
            elif resizing:
                if currentDrawColor == (0, 0, 0):
                    cv2.line(img, (index_x, index_y),
                             (thumb_x, thumb_y), eraseColor, 3)
                    ...
                    cv2.line(img, (index_x, index_y),
                             (thumb_x, thumb_y), currentDrawColor, 3)
                    length = math.hypot(index_x - thumb_x, index_y - thumb_y)
                    print("Brush thickness: " + str(length // 2))
                    brushThickness = length // 2
                    selectedThickness = brushThickness
            elif colorPicking:
                # Current color is black (eraser color)
                if currentDrawColor == eraseColor:
                    ...
                    # Check if hue_x is within the width of the image
                    if hue_x < hue_image.shape[1]:
                        # Access the pixel value at (0, hue_x) and convert it to a tuple of integers
                        # Set the RGB color
                        lastDrawColor = tuple(int(c)
                                              for c in hue_image[0, hue_x])
                        currentDrawColor = lastDrawColor
...
def load_hotkeys(config_path="hotkeys.json"):
    current_directory = os.path.dirname(os.path.abspath(__file__))
    hotkeys_path = os.path.join(current_directory, config_path)
